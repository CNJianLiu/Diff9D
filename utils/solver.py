...
class Solver(gorilla.solver.BaseSolver):
    def __init__(self, model, data_mode, loss, dataloaders, logger, cfg, start_epoch=1, start_iter=0):
        ...
        # self.lr_scheduler = optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=1e-6, max_lr=1e-4,
        #                                     step_size_up=10000, mode='triangular', cycle_momentum=False)
    ...
    def solve(self):
        while self.epoch <= self.cfg.max_epoch:
            ...
            if self.epoch % 100 == 0:
                self.lr_scheduler = optim.lr_scheduler.CyclicLR(self.optimizer, base_lr=0.8 ** (self.epoch/100) * 1e-6, max_lr=0.8 ** (self.epoch/100) * 1e-4, step_size_up=1000 * 2000 // 200, mode='triangular', cycle_momentum=False)
    ...
    #         # 'rgb1': torch.cat([syn_data['rgb1'], real_data['rgb1']], dim=0),
    ...
    with tqdm(total=len(dataloder)) as t:
        for i, data in enumerate(dataloder):
            ...
            if 'rgb' in  data.keys():
                ...
                # end_points = model(inputs)
                # end_time1 = time.time()
                # print('FPS : {}'.format(end_time1 - start_time1))
...
def draw_box_to_image(data, result,  data_name, img_id):
    ...
    out_dir = "/mnt/HDD2/lj/Diff9D/log1/vis_real/"
