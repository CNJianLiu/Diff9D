...
class PointNetfeat(nn.Module):
    def __init__(self, num_points=1024, global_feat=False, out_dim=512, feature_transform=True, **args):
    ...
    def forward(self, x, **args): # b*3*1024
        n_pts = x.shape[2]
        trans = self.stn(x)  # b*3*3
        x = x.transpose(2, 1) # b*1024*3
        x = torch.bmm(x, trans) # b*1024*3
        x = x.transpose(2, 1) # b*3*1024
        x = F.relu(self.conv1(x))
        ...
        if self.global_feat:
            ...
            x1 = x.view(-1, self.out_dim, 1).repeat(1, 1, n_pts) # bs*512*1024
            return x, torch.cat([x1, pointfeat], 1)

# NOTE: removed BN
class ShapeNetfeat(nn.Module):
    def __init__(self, num_points=1024, global_feat=False, out_dim=512, feature_transform=True, **args):
        super(ShapeNetfeat, self).__init__()
        self.num_points = num_points
        self.out_dim = out_dim
        self.feature_transform = feature_transform
        self.stn = STN3d()
