...
class TrainingDataset(Dataset):
    ...
    def get_sym_info(self, c, mug_handle=1):
        ...
        if c == 'bottle':
            ...
            sym = np.array([0, 1, 0, 0], dtype=np.int_)  # for mug, we currently mark it as no symmetry
    ...
    def __getitem__(self, index):
        ...
        if self.use_fill_miss:
            depth = fill_missing(depth, self.norm_scale, 1) #Ture
...
class TestDataset():
    ...
    def __getitem__(self, index):
        ...
        with open(path, 'rb') as f:
            data = cPickle.load(f)
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!MASK R-CNN ON CAMERA+REAL
        # pred_data = data
        # pred_mask = data['pred_masks']

        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!MASK R-CNN ON CAMERA
        ...
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!MASK R-CNN ON ImageNet
        # load mask-rcnn detection results
        # img_path = os.path.join(self.data_dir, data['image_path'])
        # img_path = img_path.replace('/data/real/', '/data/Real/')
        # img_path_parsing = img_path.split('/')
        # mrcnn_path = os.path.join('data/mrcnn_results/real_test', 'results_test_{}_{}.pkl'.format(
        #     img_path_parsing[-2], img_path_parsing[-1]))
        # with open(mrcnn_path, 'rb') as f:
        #     pred_data = cPickle.load(f)
        # pred_mask = pred_data['masks']

        image_path = os.path.join(self.data_dir, data['image_path']) #always
        image_path = image_path.replace('/data/real/', '/data/Real/') #always

        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! MASK R-CNN ON CAMERA OR CAMERA+REAL
        ...
        #!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!MASK R-CNN ON ImageNet
        # num_instance = len(pred_data['class_ids'])
        ...
        # for j in range(num_instance):
        #     inst_mask = 255 * pred_mask[:, :, j].astype('uint8')
        #     rmin, rmax, cmin, cmax = get_bbox(pred_data['rois'][j])
        #     mask = inst_mask > 0
        #     mask = np.logical_and(mask, depth>0)
        #     choose = mask[rmin:rmax, cmin:cmax].flatten().nonzero()[0]
        ...
        #     # instance_rgb = rgb[rmin:rmax, cmin:cmax, :].copy() * mask.astype(int)
        #     # print(instance_rgb)
        ...
        #         cat_id = pred_data['class_ids'][j] - 1 # convert to 0-indexed

        #         class_name = self.class_name_map[pred_data['class_ids'][j]]
        ...
        # ret_dict['pred_class_ids'] = torch.tensor(pred_data['class_ids'])[flag_instance==1]
        # ret_dict['pred_bboxes'] = torch.tensor(pred_data['rois'])[flag_instance==1]
        # ret_dict['pred_scores'] = torch.tensor(pred_data['scores'])[flag_instance==1]
        # ret_dict['index'] = torch.IntTensor([index])
        # return ret_dict
